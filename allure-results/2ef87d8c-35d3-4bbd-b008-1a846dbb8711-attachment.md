...
> 33 |                 expect(isLoggedIn).toBeTruthy() // Assert that the user is logged in    
  ...
  37 |                 // Handle both types of error messages: invalid credentials OR account locked
  38 |                 const isValidError = errorMessage.includes("Warning: No match for E-Mail Address") || 
  39 |                                     errorMessage.includes("Warning: Your account has exceeded allowed number of login attempts");
  40 |                 expect(isValidError).toBeTruthy() // Assert that an error message is displayed    
  41 |             }
  42 | 
  43 |     })
  44 | }
