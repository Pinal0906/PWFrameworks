...
```
TypeError: randomDataGenerator.getFirstName is not a function
```
...
> 20 | await registrationPage.setFirstName(randomDataGenerator.getFirstName())
  ...
  24 | const password = randomDataGenerator.getPassword()
  25 | await registrationPage.setPassword(password)
  26 | await registrationPage.setConfirmPassword(password)
  27 | await registrationPage.selectPrivacyPolicy()
  28 | await registrationPage.clickContinue()         
  29 |     
  30 | // Validate the cofirmation message
  31 | const confirmationMessage = await registrationPage.getConfirmationMessage()
  32 | await expect(confirmationMessage).tocontain("Your Account Has Been Created!") 
  33 | 
  34 | await page.waitForTimeout(3000) // wait for 3 seconds before closing the browser     
  35 | })
```